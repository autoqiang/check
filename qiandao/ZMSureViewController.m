...
@interface ZMSureViewController ()<UITextFieldDelegate>
{
    UITextField * field;
    UIButton * btnReset;
    NSString* yanzheng;
...
@implementation ZMSureViewController
...
-(void)ensure{
    if([field.text isEqualToString:yanzheng]){
        ZMChangePasswordViewController * zmc=[[ZMChangePasswordViewController alloc] init];
        zmc.phoneNumber=self.phoneNumber;
        zmc.yanzheng = field.text ;
        [self.navigationController pushViewController:[[ZMChangePasswordViewController alloc] init] animated:YES];
        return;
    }
    else{
        UIAlertController *alertControl = [UIAlertController alertControllerWithTitle:@"验证码错误" message:@"请重新输入验证码" preferredStyle:UIAlertControllerStyleAlert];
